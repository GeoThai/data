...
const readJsonFile = (filePath: string): object => {
  try {
    ...
    console.error(`${chalk.red(figures.cross)} ${filePath} JSON file not found.`)
    throw err
  }
}

const validate = (version: string, dataDir: string, errors: { [key: string]: string[] }) => {
  const schemaDir = path.join(process.cwd(), 'schemas', version)

  if (!fs.existsSync(schemaDir)) {
    console.error(`${chalk.red(figures.cross)} ${version} JSON schema directory not found.`)
    return
  }
  ...
  fs.readdirSync(dataDir).forEach((file) => {
    if (file.endsWith('.json')) {
      ...
      const schemaPath = path.join(schemaDir, file.replace('.json', '.schema.json'))
...
const main = async () => {
  ...
  if (versions.length === 0) {
    console.error(`${chalk.red(figures.cross)} No data found in 'data' directory.`)
    return
  }
  ...
  if (totalErrors === 0) {
    ...
    Object.entries(errors).forEach(([version, errs]) => {
      errs.forEach((err) => {
        console.log(`  ${chalk.red(figures.cross)} ${chalk.yellow.bold(version)}: ${err}`)
      })
    })
    process.exit(1)
  }
